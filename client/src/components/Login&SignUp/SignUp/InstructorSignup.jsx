...
const SignUpService = async (request) => {
  try {
      ...
      const response = await fetch("http://localhost:8080/api/auth/InstructorSignUp", {
    ...
    return { status: response.status, data: responseRaw.id };
  } 
  catch (error) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (responseData ) {
          ...
          setInstructorId(id);
          console.log("User has been signed up successfully. Instructor ID:", instructorId);


          navigate("Registration-Page", { state: { id } });
        ...
        <MDBCol className={style.loginSide} >
         ...
         value={password}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Link to="/InstructorSignin">Sign in</Link>
          </div>
        </MDBCol>
      </MDBRow>
    </MDBContainer>
