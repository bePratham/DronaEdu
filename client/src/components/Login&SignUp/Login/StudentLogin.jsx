...
function StudentLogin() {
  ...
  const { loginDetail } = useUser(); //take and update
  ...
  const login = async (requestBody) => {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/auth/studentsignIn", {
      method: 'POST',    
      headers: {
    ...
    console.log(student)
      if (response.status >= 200 && response.status < 300) {
        ...
        localStorage.setItem('userId', student.studentDetail.id);
        loginDetail(student.studentDetail.id);
